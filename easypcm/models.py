...
class WorkOrderRow(Base):
    ...
    # Execução (serão preenchidos no fechamento)
    ...
    abertura_em: Mapped[str] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    fechamento_em: Mapped[str | None] = mapped_column(
...
class ChatState(Base):
    ...
    mode: Mapped[str] = mapped_column(
        String, default="IDLE"
    )  # IDLE, OPEN_FLOW, CLOSE_FLOW, CONSULT_FLOW

    step: Mapped[str] = mapped_column(
        String, default=""
    )  # qual pergunta estamos esperando

    os_id: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # Dados temporários para fluxo de abertura
    ...
    temp_maquina_parada: Mapped[str] = mapped_column(String, default="")
        # Dados temporários para fluxo de fechamento
